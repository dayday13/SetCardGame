...
public class Table {
    ...
    public Table(Env env, Integer[] slotToCard, Integer[] cardToSlot) {
        ...
        for(int i=0; i < tokens.length; i++)
            tokens[i] = new LinkedList<>();
        this.waitingPlayers = new LinkedBlockingQueue<>();
    }
    ...
    public boolean removeToken(int player, int slot) {
        ...
        {
            tokens[slot].remove(tokens[slot].indexOf(player));
    ...
    public List<Integer> getSlotsWithTokens(int playerId){
        List<Integer> output = new LinkedList<>();
        for(int i=0; i < tokens.length ; i++)
        {
            if(tokens[i].contains(playerId))
                output.add(i);
        }
        return output;
    }

    public int getNumbersOfTokens(int playerId){
        int output = 0;
        for(int i=0; i < tokens.length ; i++)
        {
            if(tokens[i].contains(playerId))
    ...
    public List<Integer> getCardsWithTokens(int playerId)
    {
