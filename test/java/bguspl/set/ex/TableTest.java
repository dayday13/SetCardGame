...
class TableTest {
    ...
    void setUp() {
        ...
        /////
        Player[] players = new Player[env.config.computerPlayers];
    ...
    void placeToken() {
        ...
        //no tokens are placed for player on table
        assertEquals(table.getSlotsWithTokens(player.id).size(), 0);

        //placing tokens for our player
        table.placeToken(player.id, 0);
        table.placeToken(player.id, 2);

        //testing that our player's tokens were added
        assertEquals(table.tokens[0].get(0), player.id);

        assertEquals(table.tokens[2].get(0), player.id);

    }

    @Test
    void getSlotsWithTokens() {

        //no tokens are placed for player on table
        assertEquals(table.getSlotsWithTokens(player.id).size(), 0);

        //placing a token for our player
        table.placeToken(player.id, 3);

        List<Integer> listPlayersTokens = table.getSlotsWithTokens(player.id);

        //making sure out token was added for the same slot
        assertEquals(listPlayersTokens.size(), 1);

        //testing that our player's token was added
        assertEquals(listPlayersTokens.get(0), 3);
