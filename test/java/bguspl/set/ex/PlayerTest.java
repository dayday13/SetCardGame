...
class PlayerTest {
    ...
    void score() {
        ...
        // when player created score should be 0
        ...
        // check that ui.setScore was called with the player's id and the correct score
        verify(ui).setScore(eq(player.id), eq(1));
    }

    @Test
    void setNeedsPenalty(){

        //when created player should be set to false
        assertFalse(player.needsPenalty);

        //setting field to true
        player.setNeedsPenalty(true);

        //should be true
        assertTrue(player.needsPenalty);

        //returning player to its previous state
